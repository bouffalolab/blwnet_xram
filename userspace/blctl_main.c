...
#define epln(fmt, ...) fprintf(stderr, fmt "\n", ##__VA_ARGS__)

static blctl_handle_t ctl = NULL;

void usage(const char *prog)
{
    fprintf(stderr, "blctl version " VERSION "\n\n"
                    "Usage: %s <cmd> [cmd arg]\n"
                    "          <cmd>:\n"
...
int connect_ap(const char *ssid, const int ssid_len, const char *password, const int pwd_len)
{
    ...
    if ((ret = blctl_rnm_msg_expect_simple(ctl, (void *)&cmd, sizeof(cmd), &status, 5 * 1000))) {
        epln("error: ret %d", ret);
        return ret;
    }
...
int start_ap(const char *ssid, const int ssid_len, const char *password, const int pwd_len, uint8_t is_open)
{
    ...
    memcpy(cmd.password, password, pwd_len);
	cmd.is_open = is_open;
    if ((env_channel = getenv("CHANNEL"))) {
        channel = atoi(env_channel);
...
static void wildcard_msg_handler(void *msg, void *arg)
{
    ...
    default:
        printf("%s: no handler for type %u\n", __func__, hdr->type);
        break;
    }
