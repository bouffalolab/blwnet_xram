...
int blctl_ota(blctl_handle_t handle, const char *fw_bin_path)
{
    ...
    if ((ret = blctl_ota_start(handle))) {
        fprintf(stderr, "START OTA failed with code %d\n", ret);
        goto cleanup;
    }
    printf("OTA START DONE\n");
    ...
    while (write_offset < bin_size) {
        ...
        if (read_ret == program_size) {
            printf("To program offset %u/%lu%s, ", write_offset, bin_size, is_last_part ? "(LAST)" : "");
            if ((ret = blctl_ota_program_part(handle, write_offset, read_buf, program_size))) {
                fprintf(stderr, "program erred with code %d\n", ret);
                goto cleanup;
            }
            printf("Done\n");
            write_offset += program_size;
        } else {
            fprintf(stderr, "read ota file error\n");
            goto cleanup;
        }
    }

    if ((ret = blctl_ota_commit(handle))) {
        fprintf(stderr, "commit erred with code %d", ret);
        goto cleanup;
    } else {
        printf("OTA finished\n");
        ret = 0;
